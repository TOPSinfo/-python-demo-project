...
from user_auth_sys.custom_decorators import check_permission, is_logged_in, is_user_approved
        ...
        if user is not None:
            ...
            if flag:
                if user.is_active:
                    ...
                    if user.is_staff and user.status == 1:
                        return redirect("user_profile")
                    elif user.is_staff and user.status == 2:
                        return redirect("dashboard_view")
                    elif user.is_staff and user.status == 3:
                        return redirect("user_profile")
                    elif user.is_superuser and user.status == 0:
                        return redirect("user_profile")
                    else:
                        return redirect("dashboard_view")
                else:
...
class DashboardView(View):
    ...
    @method_decorator(is_user_approved)   
    def get(self, request, *args, **kwargs):
        context = {}
